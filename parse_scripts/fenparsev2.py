...
def fen2mlinput(fen_list, num_legal_per_board=5, num_illegal_per_board=5):
    ...
    df = pd.DataFrame([], columns=['move_info', 'preboard', 'postboard', 'x', 'y'])
    for fen in fen_list:
        try :
            iboard, to_move = fen2iboard(fen)
            #rotate if necessary
            ...
            for i in range(0, num_legal_per_board) :
                ...
                df = df.append(pd.DataFrame([[MoveInfo(iboard, legal_moves[i]), iboard, post_board, new_legal, 1]],                                            columns=['move_info', 'preboard', 'postboard', 'x', 'y']))
            ...
            for i in range(0, num_illegal_per_board):
                ...
                if (type(next_move) == tuple): # will be none when it is out of 
                    ...
                    df = df.append(pd.DataFrame([[MoveInfo(iboard, next_move), iboard, post_board, new_illegal, 0]],                                                columns=['move_info', 'preboard', 'postboard', 'x', 'y']))
        except IndexError:
            pass
